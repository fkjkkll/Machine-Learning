...
Created on Tue Aug 31 12:49:28 2021
@author: lee
"""
import numpy as np
...
#     if not hasattr(dropout, 'mask'):
#         dropout.mask = None
#     dropout.mask = np.ones_like(x)
#     drop_index = np.random.rand(x.shape[1])
#     drop_index = (drop_index > keep_ratio)
#     dropout.mask[:, drop_index] = 0
#     return x * dropout.mask
...
for i in range(1, train_count + 1):
    ...
    grad_w = np.matmul(batch_train_x.T, (np.matmul((h - batch_train_y), v.T) * relu.mask * dropout.mask))
    v = v - lr * grad_v
    w = w - lr * grad_w
